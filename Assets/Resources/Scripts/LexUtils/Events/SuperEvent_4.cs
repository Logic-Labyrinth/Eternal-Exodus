...
namespace LexUtils.Events {
    public class SuperEvent<T0, T1, T2, T3> : UnityEvent<T0, T1, T2, T3> {
        readonly Dictionary<string, UnityEvent<T0, T1, T2, T3>> channels = new();

        public UnityEvent<T0, T1, T2, T3> Get(string channel = "") {
            channels.TryAdd(channel, new UnityEvent<T0, T1, T2, T3>());
